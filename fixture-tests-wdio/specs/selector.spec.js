...
describe('Mode A wdio fixture — selector not found', function () {
  ...
  it('clicks #non-existent-button (deterministic fail — selector absent)', async function () {
    await browser.url(siteUrl);
    const el = await browser.$('#non-existent-button');
    // Click on absent element deterministically throws.
    await el.click();
  });
});
