...
3. **onDecision wire verification (post-drop-retry).** Have the agent call `qa-debug_qa_request_give_up`. Output Channel "QA Debug Companion" should show `[qa-debug-lm] qa-debug_qa_request_give_up called session=...` plus the canonical `[decision-router] commit ... by=agent` audit row. Status-bar hides within 500ms, mocha exits cleanly. (Retry and close-browser verbs were removed; re-running is the user's action via Test Explorer ▶.)
