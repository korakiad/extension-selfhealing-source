...
2. (Highest value) **v5.8 Thread 1 resolution verification.** Re-run fixture-tests-wdio/specs/selector.spec.js (current spec has `#login-btnxxx` — intentional bad selector). When agent calls qa-debug_qa_get_failure_context, failing_assertion MUST contain the ACTUAL wdio error (something like "element ('#login-btnxxx') still not displayed" — wdio v8's element-not-found error text). It must NOT be the defensive placeholder "(test marked failed but Mocha did not capture an error...)". Also stack_trace.frames must be populated, not empty.

3. **v5.6 onDecision wire verification (post-v5.7 audit-prefix update).** Have agent call qa-debug_qa_request_retry. Output Channel "QA Debug Companion" should show: [qa-debug-lm] qa-debug_qa_request_retry called session=... + the canonical [decision-router] commit ... by=agent audit row. Status-bar hides within 500ms, mocha respawns. Repeat for qa-debug_qa_request_give_up (no respawn, clean exit). Note: v5.7 migrated qa-debug verbs from MCP to LM Tool API; [qa-debug-mcp] log prefix is replaced with [qa-debug-lm] for these tools; the onDecision callback layer is now collapsed (commit happens directly inside the tool's invoke).
