...
When the request is to turn a TestRail case (or BDD / manual steps) into automation, follow the **test-script-orchestrator** skill. Read the TestRail case through `qaTestRailGet` first, learn the workspace pattern (reuse existing page objects, selectors, helpers, waits, assertions), then generate or update the script in the discovered framework convention. Before committing each step to code, **rehearse it against the live app** through `qa-debug-cdp` — perform the action and watch the real result (does the modal close, must you scroll, does the list settle) — so the script encodes observed behavior, not assumed behavior.

Use live-browser MCP only through `qa-debug-cdp`, which is attached to the current Live Inspect Session's specific CDP port — it does not launch a browser itself. If no Live Inspect Session is active, work repo-first and ask the QA to run **QA Debug: Inspect App** (Web / Electron / OpenFin) or **QA Debug: Attach Existing Inspect App (CDP Port)** before any browser/MCP step. After launch/attach, wait for the QA to confirm the app is logged in and at the desired starting state. If MCP inspection is ambiguous, pause, ask the QA one concrete question, then continue from the same step. Do not invent selectors or write an action you have not watched take effect — rehearsing non-destructive actions to observe their result is expected and is not guessing, but pause and ask the QA when a target or outcome stays unclear after rehearsing, and get the QA's go-ahead before any mutating or irreversible action (submit, delete, pay, send).
