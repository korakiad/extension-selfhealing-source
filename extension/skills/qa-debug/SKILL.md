...
**Verify the `qa-debug-cdp` browser tools are visible before you investigate.** Once the chrome selection has committed (Step 1b), confirm at least one `qa-debug-cdp` `browser_*` tool actually appears in your tool registry. If **no** `qa-debug-cdp` `browser_*` tool is present, the server is not running — do **not** silently fall back to reading source, and do **not** substitute a different `browser_*` server. Stop and tell the user:
...
**Use the `qa-debug-cdp` server — and ONLY that one.** The extension registers its CDP-attached playwright-mcp under the name **`qa-debug-cdp`**, so its tools appear as `mcp_qa-debug-cdp_browser_*` (host variants: a vendor-namespaced `com.microsoft/qa-debug-cdp/browser_*`, or Claude-Code-style `mcp__qa-debug-cdp__browser_*`). Find the `browser_*` tools whose **server segment is `qa-debug-cdp`** and call those.

> ⚠️ **If you ALSO see `browser_*` tools under a different server — e.g. `Playwright` / `playwright` (`mcp_playwright_browser_*`) — do NOT use them.** That is a separate playwright-mcp the QA may have installed on their machine; it drives a freshly-launched, EMPTY browser, not the held failing one. Inspecting it yields a confident-but-wrong diagnosis. Only `qa-debug-cdp` is attached (over CDP) to the held browser at `cdp_ws_url`.

There is no connect/attach tool — the extension already pointed `qa-debug-cdp` at the held browser when the chrome was selected; just call its `browser_snapshot` to inspect it, then use the rest of its surface — DOM, in-page JS, network, screenshots, plus interactive tools when read-only can't disambiguate. Investigation order is up to you (degrees of freedom: medium). Prefer read-only moves first; reach for interactive tools only when read-only can't answer.
...
2. **Propose the fix in chat** — file + line and the concrete change, enough that the user can paste it. For a product defect that's a source file; for a stale assertion that's the spec. **Surface it; don't apply it** unless the session is in autopilot / auto-approve mode (then you may edit and say so). *(This propose-first gate covers a fix **you** diagnosed in path A. A change the user explicitly dictated under Step 0 Option B is their instruction — apply exactly that, no separate approval round.)*
   - **Selector fixes:** if the fix is a selector change and the right DOM node isn't obvious from `browser_snapshot` alone, **invoke the `identify-element` skill before writing the diff** — the picker has the QA click the target in the held browser and returns verification-ready facts (computed role/name, an injected marker, match-counted CSS candidates) to verify against the live page and build a project-matched locator. Don't guess selectors when you can ask the QA visually.
