...
> **STATUS UPDATE (v5.16, 2026-05-30):** This document's header is v5.14. Later feature work post-dates it and its per-feature deltas live in the matching `PLAN-*.md` (cdp-port-discovery, runtime-tab-orient, cdp-electron-shim, env-leak-scrub, stop-button-process-group-kill, update-checker). Where this doc and the PLANs disagree on the tool surface, the PLANs win. Concretely: the live qa-debug verb set is **{ qa_get_failure_context, qa_discover_chromes, qa_select_chrome }** — the verdict verbs `qa_request_give_up` / `qa_propose_mark_passed` / `qa_propose_abort_suite` were **removed 2026-05-31** (a pause is now a pure inspection hold — no verdict to commit; the QA re-runs from Test Explorer ▶ or ends the run with Stop), as were the older `qa_request_retry` / `qa_propose_close_browser` (still referenced in §3.2 / §3.4 / §6 below). The browser model is **Mode C** (test-framework-owned Chrome discovered via CDP port probe), not the Mode A/B in older sections.
